...
```text
请在首页实现“最新科普文章”模块，但不要把文章简单堆砌成 4–6 张同样的大卡片。

设计目标：
让该模块形成清晰的信息层级，采用“1 个主推文章 + 3 条轻量文章列表”的结构。

重点修改：
- app/page.tsx 或 src/app/page.tsx
- lib/articles.ts
- components/ArticleCard.tsx
- 如有需要，新增 components/FeaturedArticleCard.tsx
- 如有需要，新增 components/CompactArticleItem.tsx
...
2. 模块描述：
   持续更新食品安全、营养、标签与热点解读内容。

3. 使用 lib/articles.ts 中的 getLatestArticles(limit) 读取最新文章。
   如果该函数不存在，请先实现。

4. 读取 4 篇文章，按 updatedAt 倒序排列。
   - 第 1 篇作为主推文章；
   - 第 2–4 篇作为右侧轻量列表。
...
11. 模块入口：
   - 如果项目已有 /articles 总列表页，标题右侧或底部增加“查看全部文章”，链接到 /articles。
   - 如果没有 /articles 页面，请不要创建无效链接。
   - 可以改为链接到“内容导航”或暂时不显示查看全部入口。

12. 视觉要求：
   - 保持当前绿色、米白色食品科普风格；
   - 主推文章和轻量列表要有层次差异；
   - hover 时主推卡片和轻量列表项都应有轻微反馈；
   - 不要显得像文章堆砌。
